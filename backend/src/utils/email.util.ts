...
export function prepareEmailToSummarize(email: EmailDetailDto): string {
  let cleanBody = email.body.textBody || '';

  if (!cleanBody && email.body.htmlBody) {
    cleanBody = convert(email.body.htmlBody, {
      wordwrap: false,
      selectors: [
  ...
  if (!cleanBody.trim()) {
    cleanBody = 'No content provided.';
  }

  return `
    From: ${email.headers.from}
    Subject: ${email.headers.subject}
    Snippet: ${email.snippet} (This is a preview generated by Gmail)
